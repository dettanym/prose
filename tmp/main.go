...
func printTags(s model.Span) {
	...
	if strings.Contains(strings.ToLower(s.OperationName), "encode") ||
		strings.Contains(strings.ToLower(s.OperationName), "decode") {
	...
	tags.Sort()
	for _, kv := range tags {
		if strings.Contains(kv.GetKey(), envoyfilter.PROSE_SIDECAR_DIRECTION) ||
			strings.Contains(kv.GetKey(), "svcName") {
			fmt.Printf("Golang filter with key %s value %s\n", kv.GetKey(), kv.GetVStr())
...
// TODO: Change the second input arg as spans can have duplicate span IDs
func parseSpan(spanID model.SpanID, spanStore map[model.SpanID]model.Span) error {
	...
	// Run this function only on spans generated by our golang filter
	// also gets whether the golang filter is inbound / outbound and encode / decode
	spanType, err := isGolangFilterSpan(span)
	if err != nil {
		// TODO: We don't need to return an error if it's just not a golang filter span
		//  --- we just go on as usual to the next span
		log.Println(err)
		return nil
	}
	...
	parentIsEnvoyProxy := ok && value.GetVStr() == "proxy"
	...
	if spanType.isInbound {
		// parent is the callee SP
		if spanType.isDecode {
...
type GolangFilterSpanType struct {
	isInbound bool // TODO: replace with SidecarDirection - it's currently within internal/common
	isDecode  bool
}

func isGolangFilterSpan(span model.Span) (*GolangFilterSpanType, error) {
	// skip Envoy's inserted spans with operation name of the form "router {inbound,outbound}|{port}||{egress,ingress}"
	//!(strings.Contains(operationName, "router") &&
	//	(strings.Contains(operationName, "egress") || strings.Contains(operationName, "ingress")))

	operationName := strings.ToLower(span.OperationName)
	svcName := strings.ToLower(span.GetProcess().GetServiceName())
	// crude checks based on our service and operation names
	if strings.Contains(svcName, "golang-filter") {
		spanType := &GolangFilterSpanType{
			isDecode: strings.Contains(operationName, "encode"),
		}

		// get whether the golang filter is inbound / outbound using a tag
		var tags model.KeyValues
		tags = span.GetTags()
		inboundOutbound, ok := tags.FindByKey(envoyfilter.PROSE_SIDECAR_DIRECTION)
		if !ok {
			return nil, fmt.Errorf("could not find a key to indicate the direction (inbound or outbound) of golangfilter span")
		}
		spanType.isInbound = strings.Contains(strings.ToLower(inboundOutbound.GetVStr()), "inbound")
		return spanType, nil
	}
	return nil, fmt.Errorf("not a GolangFilterSpan")
}