...
func printTags(s model.Span) {
	...
	// Step 1: Identify whether this span was generated by Golang filter using PROSE_* type tag.
	isProseSpan := false
	for _, kv := range tags {
	...
	if isProseSpan {
		...
		// Step 2: Print the name and value of every PROSE_* tag in this span.
		for _, kv := range tags {
			if strings.HasPrefix(kv.GetKey(), "PROSE_") {
...
func parseSpan(spanID model.SpanID, spanStore map[model.SpanID]model.Span) error {
	...
	// Step 3: Classify the span type using PROSE_SIDECAR_DIRECTION and PROSE_DATA_FLOW tags.
	// The three span types and their mapping:
	//   INBOUND  + DECODE_DATA → inbound request span  → PURPOSE_OF_USE_DIRECT
	//   OUTBOUND + DECODE_DATA → outbound request span → DATA_SHARING
	//   OUTBOUND + ENCODE_DATA → outbound response span → PURPOSE_OF_USE_INDIRECT
 
	sidecarDirectionTag, hasSidecarDirection := tags.FindByKey("PROSE_SIDECAR_DIRECTION")
	if !hasSidecarDirection {
		return fmt.Errorf("could not find PROSE_SIDECAR_DIRECTION tag on span %s — not a Prose span\n", spanID)
	}
 
	dataFlowTag, hasDataFlow := tags.FindByKey("PROSE_DATA_FLOW")
	if !hasDataFlow {
		return fmt.Errorf("could not find PROSE_DATA_FLOW tag on span %s\n", spanID)
	}
 
	isInbound := strings.Contains(sidecarDirectionTag.VStr, "INBOUND")
	isDecode := dataFlowTag.VStr == "DECODE_DATA"
	isEncode := dataFlowTag.VStr == "ENCODE_DATA"
 
	// Only DECODE_DATA and ENCODE_DATA spans carry PII results from processBody.
	// DECODE_HEADERS and ENCODE_HEADERS spans only have direction tags, no PII data.
	if !isDecode && !isEncode {
		return nil // nothing to parse for header-only spans
	}
